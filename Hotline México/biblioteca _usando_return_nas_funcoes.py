...
def procura_autor(biblioteca,autor):
    ...
    for libro in biblioteca:
        if libro[1] == autor:
            libros.append(libro)
    if libros == []:
        return "Não temos livros desse autor em nossa biblioteca!"
    else:
        return f"O autor escreveu esses livros:\n\n{libros}"
def procura_genero(biblioteca,genero):
    ...
    for libro in biblioteca:
        if genero in libro[3]:
            libros.append(libro)
    if libros == []:
        return "Não temos livros desse genero em nossa biblioteca!"
    else:
        return f"Os livros desse gênero são:\n\n{libros}"
...
while True:
    ...
    if escolha=='autor':
        ...
        print(procura_autor(biblioteca,autor))
    if escolha=='genero':
        genero = input("Insira o genero: ")
        print(procura_genero(biblioteca,genero))