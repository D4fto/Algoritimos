...
class Player(Personagem):
    def __init__(self, spr, loc, tam, escala):
        ...
        self.colide_ = Colide(pygame.transform.scale(self.image_colide,(self.image_colide.get_width()//2,self.image_colide.get_height()//2)),self)
        lista_c[2].add(self.colide_)
        self.player_colidido=0
    ...
    def update(self):
        if self.player_colidido==1:
            exit()
        if len(pygame.sprite.spritecollide(self.colide_,lista_c[1],True)):
            self.player_colidido=1
        self.andar()
